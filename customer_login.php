...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['google_id_token'])) {
        ...
        try {
            ...
            if ($payload) {
                $email = $payload['email'];
                $stmt = $conn->prepare("SELECT id, username, user_type, first_name, last_name FROM users WHERE email = ? AND user_type = 'customer'");
                ...
                if ($user) {
                    ...
                    $error = "No customer account found with this Google email. Redirecting to registration...";
